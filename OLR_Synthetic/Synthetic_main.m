...
% the input data
rng(1,'twister');
pro_a = 0.5 + (-1) * rand(col,T_Horizon);
% the output
rng(1,'twister');
x_zero = randn(col, 1);
rng(1,'twister');
var_noise = 0.04 * randn(1, T_Horizon);
b_out = zeros(1, T_Horizon);
for t = 1:T_Horizon
    b_out(1,t) = pro_a(:,t)' * x_zero + var_noise(1,t); 
end
f_opt = data_process(T_Horizon, b_out, pro_a, col);

%% return the regret of DPSDA-C
% [C_Reg_e1, C_barReg_e1, all_C_Reg_e1, all_C_barReg_e1]...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(1), Alg_Index(1),...
%     b_out, pro_a, col, f_opt); % non-private
% save('result_C1.mat', 'C_Reg_e1', 'C_barReg_e1', 'all_C_Reg_e1', 'all_C_barReg_e1');
% 
% [C_Reg_e2, C_barReg_e2, all_C_Reg_e2, all_C_barReg_e2]...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(2), Alg_Index(1),...
%     b_out, pro_a, col, f_opt); % epsilon = 0.6
% save('result_C2.mat', 'C_Reg_e2', 'C_barReg_e2', 'all_C_Reg_e2', 'all_C_barReg_e2');
% 
% [C_Reg_e3, C_barReg_e3, all_C_Reg_e3, all_C_barReg_e3]...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(3), Alg_Index(1),...
%     b_out, pro_a, col, f_opt); % epsilon = 0.2
% save('result_C3.mat', 'C_Reg_e3', 'C_barReg_e3', 'all_C_Reg_e3', 'all_C_barReg_e3');
% 
% [C_Reg_e4, C_barReg_e4, all_C_Reg_e4, all_C_barReg_e4]...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(4), Alg_Index(1),...
%     b_out, pro_a, col, f_opt); % epsilon = 0.1
...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(1), Alg_Index(2),...
%     b_out, pro_a, col, f_opt); % non-private
...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(2), Alg_Index(2),...
%     b_out, pro_a, col, f_opt); % epsilon = 0.6
...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(3), Alg_Index(2),...
%     b_out, pro_a, col, f_opt); % epsilon = 0.2
...
%     = Synthetic_call(Num_Nodes, T_Horizon, sigma(4), Alg_Index(2),...
%     b_out, pro_a, col, f_opt); % epsilon = 0.1
