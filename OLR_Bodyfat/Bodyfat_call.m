function [Reg, bar_Reg, all_Reg, all_bar_Reg] = Bodyfat_call(Num_Nodes, T_Horizon, sigma, Alg_Index,...
    f_opt, feature_Rand, label_Rand, col)
%SYNTHETIC_CALL using to compute regrets of algorithms
% Reg: the pseudo-regret (3)
% bar_Reg: the pseudo-regret (19)
% all_Reg: the pseudo-regret (3) over 20 Monte Carlo tests
% all_bar_Reg: the pseudo-regret (19) over 20 Monte Carlo tests
...
if Alg_Index == 1  % compute the regrets of DPSDA-C  
    for ave = 1:Monte_Carlo_tests
       ...
       for t = 1:T_Horizon
          [Reg_F(t), bar_Reg_F(t)] = DPSDA_C(col, Num_Nodes, t, feature_Rand(1:t,:),...
              label_Rand(1:t), f_opt(t), A(:,:,1:t), sigma, lb, ub); 
       end
       all_Reg(ave,:) = Reg_F;
...
if Alg_Index == 2  % compute the regrets of DPSDA-PS
    for ave = 1:Monte_Carlo_tests     
       ...
       bar_Reg_F = zeros(1, T_Horizon);
       for t = 1:T_Horizon
          [Reg_F(t), bar_Reg_F(t)] = DPSDA_PS(col, Num_Nodes, t, feature_Rand(1:t,:),...
              label_Rand(1:t), f_opt(t), A(:,:,1:t), sigma, lb, ub);
       end
       all_Reg(ave,:) = Reg_F;
       all_bar_Reg(ave,:) = bar_Reg_F;
